...
- 前向传播（边）：边侧完成模型尾层处理并完成loss计算，前向传播阶段完成。
...
其中上图中图1为步骤1生成的两级逻辑流水、图2为步骤2合并后的最终流水方案。步骤2合并边侧两级流水时出现了任务冲突，优化阶段时按FS-FE-BS-BE的执行顺序重排。优化的依据是此执行顺序可以增大可容忍边云通信时延：以样本3的前向传播通信为例，其通信时间可以由样本5的前向计算时间掩盖，提升了可容忍通信时延，从而在拉远收敛场景下减小算效损失。
